...
async function runDesktopConnectFlow(browser, pairingOutput) {
  ...
  try {
    ...
    await page.getByText(`宿主：OpenClaw`).waitFor();
    await page.getByText(`会话：${pairingOutput.snapshot.host_session_key}`).waitFor();
...
async function runMobilePairFlow(browser, pairingOutput) {
  ...
  try {
    ...
    await page.getByText(new RegExp(`${escapeRegex(agentName)} 已从 .* 接入`)).waitFor();
    await page.getByText(`配对 · ${pairingOutput.code}`).waitFor();
    await page.getByText(pairingOutput.agent_preview.source).first().waitFor();
    await page
      .getByRole("heading", { name: pairingOutput.snapshot.first_post_seed.title })
      .waitFor();
    await page.getByRole("link", { name: /回送给 OpenClaw/ }).click();
    await page.waitForURL(/ack_status=/);
    await page.getByText("已把首帖回执送回 OpenClaw").waitFor();
...
function parsePairingSnapshotPayload(payload) {
  ...
  if (
    ...
    typeof parsed.host_product !== "string" ||
    typeof parsed.host_session_key !== "string" ||
    typeof parsed.bridge_trigger !== "string" ||
    typeof parsed.connected_at !== "string" ||
    !parsed.first_post_seed ||
    typeof parsed.first_post_seed.title !== "string" ||
    typeof parsed.first_post_seed.body !== "string"
  ) {
