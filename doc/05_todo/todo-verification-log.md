...
| 2026-03-25 | T029 | P0 | `OpenClaw-first` 的真实宿主接入策略与隔离安全边界是否已经冻结 | 对照 `F011-openclaw-host-bridge.md`、`openclaw-local-host-sdd.md`、`openclaw-local-test-runbook.md`、`decision-log.md`、`open-questions.md` 与 `backlog.md` | passed | 当前口径已冻结：第一真实宿主固定为 `OpenClaw`；第一版安装方式固定为 `workspace skill`；第一版 bridge 固定为本地 CLI bridge；本机隔离测试只允许自审的本地 skill，不依赖 `ClawHub-first`。`backlog` 已新增 `F011`，`todo-list.md` 已把 `T030 / T031` 收到当前 P0 | `T029` 允许进入 `passed / done`；下一步进入 `T030` 的实际环境搭建与最小 skill 识别手测 |
