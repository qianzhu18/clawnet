...
- 创始人必须能沿一条真实人工路径完成手测，而不是只依赖自动回归脚本：
  - `OpenClaw` 会话 / dashboard 触发
  - 桌面 `/connect`
  - 手机 `/pair -> /app`
- `/app` 首屏必须保留最小“宿主共存态”：
  - 明确的已接入身份
  - 明确的 `source = openclaw` 或等价宿主来源
  - 首条微播客动态仍在首屏范围内
- `/app` 内至少存在一个最小回传动作，用于证明当前不是单向 handoff，而是最小 `coexistence`
...
- 其他 claw 类宿主的接入标准先按三层冻结：
  - 宿主内 `skill / plugin`
  - 宿主外 `npm / CLI`
  - 后续再考虑托管式 `skill as a service`
- 自动回归命令 `demo:openclaw:bridge / demo:openclaw:lan` 只负责证明链路，不负责充当创始人手工入口。
- 创始人手工验收入口固定为：`OpenClaw` dashboard / 会话触发 `/clawnet_connect_bridge http://<LAN_IP>:3000`，桌面浏览器打开返回的 `connect_url`，再由手机扫描 `/connect` 页面里的 SVG 二维码进入 `/pair -> /app`。
- 当前阶段仍坚持“桌面端负责配置、移动端只负责体验”。
- 当前真实宿主阶段继续使用 `Pairing Snapshot` 作为传输合同，不等待正式后端和建表完成。
- 当前最小目标已经从“接入成功”升级为“宿主真实参与”：不只要进 `/app`，还要在 `/app` 中可见宿主来源、首条动态与回传动作。
- 进入实现前，必须同时具备：
  - SDD：`doc/04详细设计/OpenClaw宿主设计.md`
  - TDD：`doc/04详细设计/OpenClaw宿主测试计划.md`
  - Runbook：`doc/06测试/OpenClaw本地测试手册.md`
