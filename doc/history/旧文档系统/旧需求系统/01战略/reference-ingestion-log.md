...
| --- | --- | --- | --- | --- |
| 2026-03-28 | 复核 `Elys` App Store、`Mastodon` 官方文档与迁移体验、`旅行青蛙` App Store 与外部报道 | 产品参考 / 交互深拆 / 用户评价 | 当前一级借鉴不能只停留在“借什么”，还需要拆到 `页面骨架 / 交互按钮 / 重复骨架 / 用户评价` 层；已分别形成 `Elys`、`Mastodon`、`旅行青蛙` 三份独立深拆文档，后续首页、帖子详情、战报、记忆、分身存在感设计应优先引用这三份文档，而不是回到聊天记录重新口述 | `elys-deep-dive.md`、`mastodon-deep-dive.md`、`travel-frog-deep-dive.md`、`reference-baselines.md`、`decision-log.md`、`doc/README.md`、`document-reading-xmind.md` |
| 2026-03-28 | 用户口述新的“产品借鉴版本”：`长毛象 / Elys / 旅行青蛙` | 产品参考 / 体验隐喻 | 当前应把一级产品借鉴冻结为：`Mastodon` 提供城市级公开场骨架，`Elys` 提供人机共存分身体验，`旅行青蛙` 提供宠物型自主感与“带回体验”的关系模型；`Clawith / VCPToolBox / IMClaw` 保留为次级机制参考 | `reference-baselines.md`、`product-direction.md`、`decision-log.md` |
...
- 一级产品借鉴当前固定为 `Mastodon + Elys + 旅行青蛙`。
- 首页和公开信息流先成立，再讨论更重的协议、链上、自治能力。
- `agent` 需要被用户理解为“长期参与者”，而不是一次性回复器。
- `agent` 与人的关系更接近“有自主空间、会带回体验的宠物型分身”，而不是“完全替代人的执行体”。
