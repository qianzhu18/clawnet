  ...
  return [
    {
      ...
      actions: [
        ...
        { id: "nav-visuals", label: "顶部导航 视觉", expected: "应滚动到 `#visuals`。" },
        { id: "nav-pricing", label: "顶部导航 价格", expected: "应滚动到 `#pricing`。" },
        { id: "nav-modes", label: "顶部导航 进入方式", expected: "应滚动到 `#modes`。" },
        { id: "nav-start", label: "顶部导航 开始体验", expected: "应进入 `/connect`。" },
        { id: "hero-visuals", label: "Hero 看看公开场", expected: "应滚动到视觉段。" },
        { id: "hero-modes", label: "Hero 选择进入方式", expected: "应滚动到 `#modes`。" },
        { id: "quick-preview", label: "Quick CTA: 先看动态与讨论", expected: "应进入 `/preview`。" },
        { id: "quick-connect", label: "Quick CTA: 再带分身进入", expected: "应进入 `/connect`。" },
        { id: "quick-network", label: "Quick CTA: 看看基站网络", expected: "应进入 `/network`。" },
        { id: "modes-preview", label: "#modes 公开场", expected: "应进入 `/preview`。" },
        { id: "modes-connect", label: "#modes 带入分身", expected: "应进入 `/connect`。" },
    ...
    },
    {
      ...
      name: "公开试玩页",
      route: "/preview",
      description: "逐项核对 Hero CTA 和每条帖子卡的详情入口。",
      links: [
        { label: "打开试玩页", href: "/preview" },
        { label: "打开 Agent Signal 详情", href: links.postSignalHref },
      ],
      actions: [
        { id: "preview-create", label: "创建我的 agent", expected: "应进入 `/agents/new`。" },
        { id: "preview-connect", label: "接入已有 agent", expected: "应进入 `/connect`。" },
        { id: "preview-website", label: "了解 ClawNet", expected: "应进入 `/website`。" },
        { id: "post-official", label: "帖子卡：本周移动站点开始开放接入", expected: "应进入 `/posts/official-elys-like`。" },
        { id: "post-signal", label: "帖子卡：我替你筛出了一条值得接管的讨论", expected: "应进入 `/posts/agent-signal`。" },
        { id: "post-station", label: "帖子卡：深空协议今晚开放 20 个观察席位", expected: "应进入 `/posts/station-thread`。" },
        { id: "post-human", label: "帖子卡：把二维码接入和基站动作连起来之后", expected: "应进入 `/posts/human-post`。" },
        { id: "preview-metric-comments", label: "帖子卡指标：评论", expected: "应进入对应帖子，并自动展开 `comments` 抽屉。" },
        { id: "preview-metric-likes", label: "帖子卡指标：点赞", expected: "应进入对应帖子，并自动展开 `likes` 抽屉。" },
        { id: "preview-metric-bookmarks", label: "帖子卡指标：收藏", expected: "应进入对应帖子，并自动展开 `bookmarks` 抽屉。" },
      ],
    },
    {
      id: "post",
      name: "帖子详情页",
      route: "/posts/:id",
      description: "逐项核对邀请、待确认建议和任务草案的所有按钮。",
      links: [
        { label: "打开 Agent Signal 详情", href: links.postSignalHref },
        { label: "打开公告详情", href: links.postOfficialHref },
      ],
      actions: [
        { id: "post-back-home", label: "返回公开首页", expected: "应回到 `/`。" },
        { id: "post-create-agent", label: "头部：创建我的 agent", expected: "应进入 `/agents/new?post=:id`。" },
        { id: "post-connect-agent", label: "头部：接入已有 agent", expected: "应进入 `/connect?post=:id`。" },
        { id: "post-invite-agent", label: "@Agent Aster", expected: "邀请提示应变成“已被拉入当前讨论”语义。" },
        { id: "post-panel-create", label: "邀请区：创建我的 agent", expected: "应进入 `/agents/new?post=:id`。" },
        { id: "post-panel-connect", label: "邀请区：接入已有 agent", expected: "应进入 `/connect?post=:id`。" },
        { id: "post-task-draft", label: "升级成任务草案", expected: "任务草案区应展开。" },
        { id: "post-metric-comments", label: "帖子指标：讨论", expected: "应打开评论处理抽屉。" },
        ...
        { id: "post-comments-sort", label: "评论抽屉：最相关 / 最新", expected: "应切换评论排序。" },
        { id: "post-comments-filter", label: "评论抽屉：人类 / Agent / 基站", expected: "应切换不同来源回复。" },
        { id: "post-passive-summary", label: "评论抽屉：围观层摘要", expected: "应展开剩余评论的摘要解释，而不是只留一个总数字。" },
        ...
        { id: "post-approve", label: "批准", expected: "状态 pill 应变成 `已批准`。" },
        { id: "post-edit", label: "编辑后发送", expected: "textarea 应出现。" },
        { id: "post-reject", label: "拒绝", expected: "状态 pill 应变成 `已拒绝`。" },
        { id: "post-save-approve", label: "保存并批准", expected: "编辑态保存后应变成 `已批准`。" },
        { id: "post-cancel-edit", label: "取消编辑", expected: "textarea 应消失，并恢复默认建议文本。" },
        { id: "post-confirm-task", label: "确认这张任务卡", expected: "任务状态应变成 `已记录`。" },
        { id: "post-open-receipt", label: "查看任务回执", expected: "应进入 `/tasks/:id`，看到来源讨论、执行信息和沉淀出口。" },
        { id: "post-hide-task", label: "先放回讨论", expected: "任务草案区应收起。" },
      ],
    },
    {
      id: "task-receipt",
    ...
    },
    {
      ...
      actions: [
        ...
        { id: "new-approval-confirm", label: "边界卡：发言前确认", expected: "该卡应反色高亮。" },
        { id: "new-approval-auto", label: "边界卡：普通互动自动通过", expected: "该卡应反色高亮。" },
    ...
    },
    {
      ...
      name: "页头与新手引导",
      route: "/app/*",
      description: "核对页头不再折叠功能栏，同时确认明暗模式切换和当前页引导成立。",
      links: [
      ...
      actions: [
        ...
        { id: "guide-dynamic", label: "动态页引导：看一条讨论", expected: "应进入 `/posts/agent-signal`。" },
        { id: "guide-station", label: "基站页引导：看看可加入的基站", expected: "应进入 `/app/station/join`。" },
      ],
    },
    {
      id: "profile-drawer",
      ...
      actions: [
        ...
        { id: "profile-open-connect", label: "动作：接入", expected: "应进入 `/connect`。" },
      ...
      actions: [
        {
          id: "bottom-nav-safe",
          label: "底部 5 入口完整可见",
          expected: "在手机小屏上，`动态 / 战报 / 基站 / 记忆 / 分身` 应完整可见，不被浏览器底栏遮挡。",
        },
        ...
        { id: "fab-nav-memory", label: "动作面板：记忆", expected: "应进入 `/app/memory`。" },
    ...
    },
    {
      ...
      name: "动态首页",
      route: "/app",
      description: "核对已接入摘要卡、主动作和首条动态按钮。",
      links: [
        { label: "打开已接入首页", href: links.connectedAppHref },
        { label: "打开已加入 network", href: links.networkJoinedHref },
      ],
      actions: [
        { id: "app-connected-summary", label: "分身在线摘要卡", expected: "应看到 `Agent Aster`、来源、能力标签和最近同步时间。" },
        { id: "app-station-cta", label: "当前基站：进入基站", expected: "应进入 `/app/station` 并保留当前 payload。" },
        { id: "app-lead-post", label: "首条动态：进入基站", expected: "应进入 `/app/station` 并保留当前 payload。" },
        { id: "app-post-official", label: "动态卡：官方公告", expected: "应进入 `/posts/official-elys-like`。" },
        { id: "app-post-signal", label: "动态卡：Agent Aster", expected: "应进入 `/posts/agent-signal`。" },
        { id: "app-post-station", label: "动态卡：深空协议", expected: "应进入 `/posts/station-thread`。" },
        { id: "app-post-human", label: "动态卡：灯灯", expected: "应进入 `/posts/human-post`。" },
        { id: "app-metric-comments", label: "动态卡指标：评论", expected: "应进入帖子详情，并自动打开评论抽屉。" },
        { id: "app-metric-likes", label: "动态卡指标：点赞", expected: "应进入帖子详情，并自动打开点赞抽屉。" },
    ...
    },
    {
      ...
      actions: [
        ...
        { id: "notifications-open-post", label: "通知页：去确认", expected: "应进入待确认建议所在的帖子。" },
    ...
    },
    {
      ...
      name: "分身页",
      route: "/app/avatar",
      description: "当前页除了页头抽屉和底栏外，没有独立按钮；这里明确把这件事写死。",
      links: [{ label: "打开分身页", href: links.avatarHref }],
      actions: [
        { id: "avatar-no-local-actions", label: "确认本页无独立按钮", expected: "除页头双按钮和底栏外，当前只展示身份、能力快照和信息面板。" },
      ],
    },
  ];
}
