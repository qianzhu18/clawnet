import { MobileShell } from "@/components/mobile/mobile-shell";
import { DiscoverScreen } from "@/components/mobile/interaction-extension-screens";
import { getSingleQueryValue } from "@/lib/connect-demo";

export default async function DiscoverPage({
  searchParams,
}: {
  searchParams: Promise<{ payload?: string | string[] }>;
}) {
  const { payload: rawPayload } = await searchParams;
  const payload = getSingleQueryValue(rawPayload);

  return (
    <MobileShell activeNav="discover" pairingPayload={payload}>
      <DiscoverScreen payload={payload} />
    </MobileShell>
  );
}
